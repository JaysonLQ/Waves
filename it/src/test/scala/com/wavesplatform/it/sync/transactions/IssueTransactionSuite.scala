...
class IssueTransactionSuite extends BaseTransactionSuite with TableDrivenPropertyChecks {
  ...
  val supportedVersions = List(null) // add "2" after IssueTransactionV2 activation

  test("asset issue changes issuer's asset balance; issuer's waves balance is decreased by fee") {
    for (v <- supportedVersions) {
      val scriptText       = if (Option(v).isDefined) script else null
      val assetName        = "myasset"
      val assetDescription = "my asset description"
      val (balance1, eff1) = notMiner.accountBalances(firstAddress)

      val issuedAssetId =
        sender.issue(firstAddress, assetName, assetDescription, someAssetAmount, 2, reissuable = true, issueFee, script = Some(scriptText)).id
  ...
  test("Able to create asset with the same name") {
    for (v <- supportedVersions) {
      ...
      val issuedAssetId =
        sender.issue(firstAddress, assetName, assetDescription, someAssetAmount, 2, reissuable = false, issueFee, script = Some(scriptText)).id
      nodes.waitForHeightAriseAndTxPresent(issuedAssetId)
